...
class ErrorClassifierTools {
  ...
  static final String WORD_EMBEDDINGS = "/media/Data/word-embeddings/glove/glove.6B.100d-top50K.txt";
  ...
  INDArray getSentenceVector(int contextSize, String sentence, String word1, String word2) {
    ...
    int wordPos = tokens.indexOf(word1);
    if (wordPos == -1) {
      wordPos = tokens.indexOf(word2);
      if (wordPos == -1) {
        throw new RuntimeException("'" + word1 + "' or '" + word2 + "' not found: " + sentence);
      }
    }
    int pos = 0;
